...
| $\color{#ffff77}{\text{Antigravity}}$ | 3 | ![](icons/boots.png)![](icons/bow.png)![](icons/sword.png)![](icons/axe.png)![](icons/spear.png) | (Melee) Chance to give target levitation. (Bow) Arrows are not affected by gravity. (Boots) Double-jump to gain temporary levitation. | (Melee) 20% chance for Levitation I for (2 + level) seconds. (Bow) 15% chance to give the target Levitation I for (2 + level) seconds. (Boots) Double-jumping triggers Levitation (3 * level) for 2 seconds. Sneaking will cancel levitation from any source while boots are on. |
